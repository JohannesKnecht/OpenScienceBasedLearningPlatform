...
- Prefer small patches, or edit domain-specific generator/script files and run short commands. The current CS1 generator is `frontend/AnkiAICardCreationFrontend/scripts/generate-cs1-curriculum.mjs`; future domains such as ML should get their own generator or small content patch sequence instead of being appended through a huge prompt.
- From `frontend/AnkiAICardCreationFrontend`, use `npm run content:generate` for all registered curriculum generators, or `npm run content:generate-cs1` for only the CS1 generator.
